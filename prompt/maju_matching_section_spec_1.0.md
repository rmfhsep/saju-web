...
`colorToken` 필드는 현재 placeholder다. 칩 3개와 축 비교 바 뱃지 2개가 동일 팔레트로 통일되는 것만 확정되었고, 실제 색상값(hex 또는 CDS 토큰명)은 디자이너 확정 후 이 문서 § 5와 컴포넌트 상수 파일에 반영한다. 개발 착수 시 임시로 `var(--c-purple-*)` 등 기존 목업 색상을 하드코딩하지 말고 별도 상수 파일(`reasonChipColors.ts` 등)로 분리해 나중에 일괄 교체 가능하게 구현할 것을 권장한다.
