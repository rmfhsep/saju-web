    ...
    <Screen>
      ...
      <div className="flex-1 px-5 pt-2 flex flex-col gap-3">
        {Array.from({ length: 8 }).map((_, i) => (
          <div key={i} className="h-[16px] rounded-[4px] bg-[#f4f4f5] animate-pulse" style={{ width: `${60 + (i % 3) * 10}%` }} />
        ))}
      </div>
    </Screen>
  )
    ...
    <Screen>
      ...
      <div className="px-5 pt-6 flex flex-col gap-4 shrink-0">
        ...
        {job ? (
          ...
          >
            <SearchIcon size={24} />
            <span className="flex-1 text-[16px] tracking-[-0.32px] text-[#1f1f1f]">{job}</span>
          </button>
        ) : (
      ...
      {job && isProfessional && (
        <div className="flex-1 min-h-0 flex flex-col gap-3 px-5">
