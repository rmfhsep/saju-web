...
const INFO_ROWS: {
  key: keyof ProfileData | "birth";
  label: string;
  format?: (data: EditData) => string;
}[] = [
  ...
  { key: "location", label: "거주지" },
  {
    key: "job",
    label: "직업",
    format: (d) => [d.job, d.jobDetail].filter(Boolean).join(" · "),
  },
  {
    key: "height",
    label: "키",
    format: (d) => (d.height ? `${d.height}cm` : ""),
  },
...
function formatBirthDisplay(
  ...
  const y = birthDate.slice(0, 4),
    m = parseInt(birthDate.slice(4, 6)),
    d = parseInt(birthDate.slice(6, 8));
  const dateStr = `${y}년 ${m}월 ${d}일`;
  return birthTime
    ? `${calLabel} ${dateStr} · ${birthTime}`
    : `${calLabel} ${dateStr}`;
}
...
export default function ProfileEditPage() {
  ...
  async function handleFiles(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setLoadingSlots(
      new Set(Array.from({ length: count }, (_, k) => activeSlot + k))
    );
    try {
      const phone = localStorage.getItem("user_phone") ?? "";
  ...
  function handleGridPointerUp() {
    if (gridRef.current && activePointerId.current != null) {
      try {
        gridRef.current.releasePointerCapture(activePointerId.current);
      } catch {
        /* noop */
      }
    }
  ...
  const slots = [
    ...data.photos,
    ...Array(Math.max(0, MAX_PHOTOS - data.photos.length)).fill(""),
  ];
    ...
    <Screen>
      ...
      <div className="flex-1 scroll-area overflow-y-auto pb-9 flex flex-col gap-12">
        ...
        <section className="mt-5 flex flex-col gap-3">
          <h2 className="text-[16px] font-semibold text-[#1f1f1f] tracking-[-0.32px] px-5">
            프로필 사진
          </h2>
          <div
            ref={gridRef}
            ...
            {data.bioTags.map((tag, idx) => (
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <span className="h-[36px] flex items-center px-4 bg-[#e9f1ff] border border-[#b6d0ff] rounded-[4px] text-[13px] font-medium text-[#1f1f1f]">
                    {tag}
                  </span>
                  <span className="text-body3Semibold underline text-[#777]">
                    수정
                  </span>
                </div>
              ...
              >
                <div className="w-full flex items-center gap-3">
                  <div className="flex-1 flex flex-col gap-0.5 items-start text-left">
                    <span className="text-[14px] font-medium text-[#1f1f1f] tracking-[-0.14px]">
                      {row.label}
                    </span>
                    <span className="text-[12px] text-[#949494]">
                      {(row.format
                        ? row.format(data)
                        : (data[row.key as keyof ProfileData] as string)) ||
                        "미입력"}
                    </span>
                  </div>
          ...
          <div className="relative bg-white rounded-[8px] p-5 w-[312px] flex flex-col gap-6">
            <div className="flex flex-col gap-2">
              ...
              <p className="text-[15px] text-[#777] tracking-[-0.3px]">
                먼저 새로운 사진을 올리고, 바꾸고 싶은 사진을 삭제해 주세요.
              </p>
            </div>
            <button
              onClick={() => setShowRequiredModal(false)}
...
function EditPhotoSlot({
  ...
  return (
    <div
      ...
      className={`relative flex-1 transition-transform ${
        dragging ? "scale-105 opacity-80 z-10" : ""
      }`}
    >
      <button
      ...
      >
        ...
        {required && (
          <span className="absolute z-10 top-2 left-2 bg-[#1a75ff] text-white text-[12px] font-medium px-[6px] py-px rounded-full leading-[1.4] pointer-events-none">
            필수
          </span>
        )}
      </button>
