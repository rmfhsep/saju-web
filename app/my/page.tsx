...
function filterSummary(u: MeUser): string | null {
  ...
  switch (u.preferredFilterType) {
    case "height": {
      if (u.preferredHeightMin == null || u.preferredHeightMax == null) return "키"
      const boundary = u.gender === "MALE" ? 170 : 200
      const maxLabel = u.preferredHeightMax >= boundary ? `${u.preferredHeightMax}+` : `${u.preferredHeightMax}`
      return `키, ${u.preferredHeightMin}~${maxLabel}cm`
    }
    case "smoking":
      return u.preferredSmoking ? `흡연 여부, ${u.preferredSmoking}` : "흡연 여부"
    case "drinking":
      return u.preferredDrinking ? `음주 빈도, ${u.preferredDrinking}` : "음주 빈도"
    case "politics":
      return u.preferredPolitics ? `정치 성향, ${u.preferredPolitics}` : "정치 성향"
    case "religion":
...
function ListCell({
  ...
  return (
    <button onClick={onClick} className="w-full flex flex-col items-start px-5 active:bg-[#fafafa]">
      <div className="w-full flex items-center gap-3 py-4">
        ...
        <span className="flex flex-col items-start gap-0.5 flex-1 min-w-0">
          <span className="text-left text-[14px] font-medium text-[#1f1f1f] tracking-[-0.14px]">{label}</span>
          {subtitle && (
            <span className="text-left text-[12px] font-normal text-[#949494] tracking-[-0.12px]">{subtitle}</span>
          )}
        </span>
      </div>
        ...
        <ListCell
          ...
          icon={
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
              <path d="M3 6h9M15 6h2M3 14h2M8 14h9" stroke="#1f1f1f" strokeWidth="1.5" strokeLinecap="round" />
              <circle cx="13.5" cy="6" r="2" stroke="#1f1f1f" strokeWidth="1.5" />
              <circle cx="6.5" cy="14" r="2" stroke="#1f1f1f" strokeWidth="1.5" />
            </svg>
          }
        />
        <ListCell
          ...
          icon={
            <svg width="20" height="20" viewBox="0 0 20 20" fill="none">
              <rect x="3.5" y="2.5" width="13" height="15" rx="2" stroke="#1f1f1f" strokeWidth="1.5" />
              <path d="M10 12.2c-2-1.3-3.2-2.4-3.2-3.7 0-1 .8-1.6 1.6-1.6.6 0 1.2.3 1.6.9.4-.6 1-.9 1.6-.9.8 0 1.6.6 1.6 1.6 0 1.3-1.2 2.4-3.2 3.7z" fill="#1f1f1f" />
            </svg>
          }
        />
        <ListCell
