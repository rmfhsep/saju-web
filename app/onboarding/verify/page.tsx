...
function PasswordInput({ value, show, onChange, onToggle, placeholder, error, success }: {
  value: string
  show: boolean
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        ...
        />
        <button type="button" onClick={onToggle} className="ml-2 text-[#b7b7b7]">
          <EyeIcon visible={show} />
        </button>
      </div>
      {error && <p className="text-[12px] text-[#ff3b30] leading-[1.4]">{error}</p>}
      {!error && success && <p className="text-[12px] text-[#1a75ff] leading-[1.4]">{success}</p>}
    </div>
  )
}
...
function VerifyForm() {
  ...
  function toggleTerm(key: TermKey) { setAgreed(prev => ({ ...prev, [key]: !prev[key] })) }
  function toggleAll() {
          ...
          <div className="flex flex-col gap-1">
            <div className="relative h-[248px] overflow-hidden">
              <div className="absolute left-[41px] top-[33px]">
                ...
                />
                <p className="absolute left-[133px] top-[17px] text-[11px] font-bold text-[#1b1c1e] whitespace-nowrap">새로운 메시지</p>
